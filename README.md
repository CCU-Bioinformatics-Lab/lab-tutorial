...
一個主題可以有好幾頁；目前有兩個主題：
「主題一 · Subclone 系統發生重建」（`sr1` / `sr2` / `sr2b`）與
「主題二 · 純度、倍體與腫瘤 DNA 比例」（`sr3` / `sr4` / `sr5` / `sr6`）。
...
|---|---|
| 研究指引 主題三以後 | 尚未開始 |
| 主題二的實測 | `sr3`–`sr6` 仍為代數、證據邊界與實作規格，LiFT-CN 尚未在真實或模擬資料上執行；最小驗證見 `sr6` 的「數學與軟體驗收條件」 |
...
`src/modules/sr*.html`，以及日後新增的任何主題 ——
