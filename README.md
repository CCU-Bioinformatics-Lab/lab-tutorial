...
| `{{cli}}…{{/cli}}` | 指令區塊（**只放真的指令與虛擬碼**，不要放公式） |
| `{{m: 式子}}` | 行內數學 → MathML |
| `{{eq}}式子{{note}}說明{{/eq}}` | 獨立一行的數學 → MathML（`{{note}}` 可省略） |
| `{{quiz:mNN}}` | 插入測驗 |

要輸出字面的 `{{` 或 `[[`，寫 `{{{` 或 `[[[`。

### 數學一律用 MathML，不要寫在 `<code>` 裡 ★

`components.css` 的 `.eq` 註解已經寫過理由：ASCII 公式只能用空白硬湊對齊，
讀者一改字級或換字型那個對齊就散掉，而且不會有任何錯誤訊息。
但手寫 MathML 太長（一個 `c_k ≥ Σ c_j` 要十幾行），所以式子很容易退化成
<code>c_k ≥ Σ_{j∈S} c_j</code> 這種「看起來像 LaTeX 的純文字」—— sr2 整頁都這樣寫過一輪。

`tools/mathml.mjs` 就是為了拿掉那個藉口：**來源寫得像 LaTeX，輸出是真正的 MathML。**
`tools/verify.mjs` 的第 13.8 項會擋掉留在 `<code>` 裡的數學。

```html
位點 {{m: m}} 的深度為 {{m: d_m}}，必要條件為 {{m: c_k ≥ \sum_{j∈S} c_j}}。

{{eq}}
E[VAF_m] = \frac{ρ · c_m · μ_m}{ρ · CN_m + 2 · (1 − ρ)}
{{note}}
二倍體、μ ＝ 1 時化簡為 {{m: E[VAF] = ρ · c / 2}}。說明裡可以放 [[term]]。
{{/eq}}
```

支援的語法**刻意只有這些**（`tools/mathml.mjs` 開頭有完整清單）：

| 寫法 | 出來的東西 |
|---|---|
| `x_i`　`x_{z_m}` | 下標（多字元要用大括號） |
| `x^2`　`e^{−λ}` | 上標 |
| `\frac{a}{b}` | 分數（真的有分數線） |
| `\sum_{m}` | 求和 —— **`{{eq}}` 時界限在符號下方，`{{m:}}` 時在右下**，這是自動的 |
| `\hat{φ}`　`\bar{x}` | 帽子／橫線 |
| `\text{支持 read 數}` | 式子裡的文字（中文與空白都會保留） |
| `\{ \}` | 字面的大括號（`{ }` 本身是分組用） |
| `\log \max \min \exp` | 直立字體的函數名 |

其餘字元自動分類：數字 → `<mn>`、運算子 → `<mo>`、其他 → `<mi>`。
運算子直接打 Unicode 就好（`≥ ≤ ≠ ∈ × ÷ · ≈`），不必寫 `\ge`。

單一字母的 `<mi>` 瀏覽器預設斜體（變數），多字元的（`VAF`、`BetaBin`）預設直立
—— 剛好符合數學排版慣例，所以不要自己指定 `mathvariant`。

**式子編不出來就 build 失敗**，不會靜靜印出原始碼。
glossary 與 quiz 的 JSON 字串裡也可以寫 `{{m: …}}`。
...
一個主題可以有好幾頁；目前有兩個主題，各兩篇：
「主題一 · Subclone 系統發生重建」（`sr1` / `sr2`）與
「主題二 · 純度、倍體與腫瘤 DNA 比例」（`sr3` / `sr4`）。
...
|---|---|
| 研究指引 主題三以後 | 尚未開始 |
| 主題二的實測 | `sr4` 全篇為代數推導，尚未在真實資料上檢驗；最小驗證見該頁「尚未解決的問題」第一條 |
...
`sr1-landscape.html`、`sr2-implementation.html`、`sr3-purity-ploidy.html`、
`sr4-haplotype-imbalance.html`，以及日後新增的任何主題 ——
